...
function SendMessage( $msg )
{
	...
	// Save Message.
	DBQuery( "INSERT INTO messages (syear,school_id," . DBEscapeIdentifier( 'from' ) . ",recipients,subject,data)
	VALUES(
		'" . UserSyear() . "',
		'" . UserSchool() . "',
		'" . $from . "',
...
function _getMessageRecipients( $recipients_key, $recipients_ids )
{
	...
	if ( $recipients_key === 'student_id' )
	{
		$names_RET = DBGet( "SELECT " . _SQLCommaSeparatedResult( DisplayNameSQL(), ', ' ) . " AS NAMES
			FROM students
			WHERE STUDENT_ID IN(" . implode( ',', array_map( 'intval', $recipients_ids ) ) . ")" );
	}
	elseif ( $recipients_key === 'staff_id' )
	{
		$names_RET = DBGet( "SELECT " . _SQLCommaSeparatedResult( DisplayNameSQL(), ', ' ) . " AS NAMES
			FROM staff
			WHERE STAFF_ID IN(" . implode( ',', array_map( 'intval', $recipients_ids ) ) . ")" );
	}
...
function GetRecipientsInfo( $user_profile, $recipients_profile = 'teacher' )
{
	...
	$users_info_sql = "SELECT STAFF_ID," . DisplayNameSQL() . " AS NAME
		FROM staff s
		WHERE s.STAFF_ID IN (" . implode( ',', array_map( 'intval', $allowed_ids ) ) . ")
		ORDER BY NAME";
