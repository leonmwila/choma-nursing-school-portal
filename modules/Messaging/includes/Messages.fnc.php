...
function MessageOutput( $msg_id )
{
	...
	// Get Message data.
	$msg_sql = "SELECT m.MESSAGE_ID, m.CREATED_AT, m.FROM, m.RECIPIENTS, m.SUBJECT, m.DATA, mxu.STATUS
		FROM messages m, messagexuser mxu
		WHERE m.SYEAR='" . UserSyear() . "'
		AND m.SCHOOL_ID='" . UserSchool() . "'
	...
	$msg_RET = DBGet(
		$msg_sql,
		[
			'CREATED_AT' => '_makeMessageDateHeader',
